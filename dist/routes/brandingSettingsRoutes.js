...
exports.createBrandingSettingsRouter = void 0;
...
const createBrandingSettingsRouter = ({ supabase }) => {
    ...
    async function deleteAssetFromStorage(url) {
        if (!url)
            return false;
        try {
    ...
    router.put('/', upload.fields([
        ...
        const { name, invoicePrefix, phone, email, street, city, state, zip, country, 
        // These are from formData. They will be strings "null" or actual URLs
        logoUrl: formDataLogoUrl, signatureUrl: formDataSignatureUrl, ...restOfBody } = req.body;
        const logoFile = req.files?.logoFile?.[0];
        const signatureFile = req.files?.signatureFile?.[0];
        const logo = req.files?.logoFile?.[0];
        const signature = req.files?.signatureFile?.[0];
...
exports.createBrandingSettingsRouter = createBrandingSettingsRouter;
