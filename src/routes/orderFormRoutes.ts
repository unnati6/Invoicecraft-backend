...
import asyncHandler from 'express-async-handler'; // Ensures async errors are caught

// Helper function to calculate totals
import { calculateOrderFormTotal } from '../utils/calculations'; // <-- Make sure this path is correct
...
export const createOrderFormRouter = ({ supabase }: RouterOptions) => {
  ...
  router.put('/:id', asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const { subtotal, discountAmount, taxAmount, grandTotal } = calculateOrderFormTotal(
        items || existingOrderForm.items,
        additionalCharges || existingOrderForm.additionalCharges,
        taxRate ?? existingOrderForm.taxRate, // Use nullish coalescing for numbers
        {
